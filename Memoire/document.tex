...
Depuis que le rendu des signaux GPS est surpassé dans les environnements \textit{indoor}, ou même en milieu urbain, plusieurs chercheurs se focalisent sur le développement d'algorithmes SLAM en utilisant des capteurs \textit{onboard (portables ?)} pour des robots ou d'autres dispositifs mobiles. De nos jours, les caméras sont des capteurs qui représentent une excellente alternative, appuyée par des facteurs de disponibilité et de moindre coût, ayant fait émergé ainsi le SLAM visuel. Les caméras RGB-D sont certes très disponibles, mais ne sont pas très \textit{on leur fait confiance, reliable, productives} dans le cas d'exposition à la lumière du soleil surtout si celle ci est \textit{puissante}, ces capteurs sont alors aveuglés par la lumière. Le télémètre est souvent considéré comme étant le capteur de cartographie le plus \textit{reliable}, mais il est bien trop énergivore, et \textit{bulky ?} et coûteux pour plusieurs dispositifs mobiles. les dispositifs électroniques, ou certains petits robots, sont contraints d'utiliser une caméra monoculaire notamment du aux contraintes mécaniques architecturales, ou à cause de la contrainte énergétique.
\begin{figure}[h]
	\centering
	\includegraphics[width=\linewidth]{fusion}
	\caption[Exemple de Fusion]{Les entrées de la carte de fusion consistent en une carte 3D de basse qualité générée par un Slam Visuel Monoculaire, et carte prioritaire de haute qualitée gnrée par des méthodes de Lidar Slam. La fsion de cartes va corriger la carte 3D en  The inputs of our map fusion include a low-quality 3D map produced by a monocular visual SLAM, and a high-precision prior map generated by lidar SLAM other methods. The map fusion rectifies the 3D map by leveraging vertical planes commonly available in both maps and outputs a more accurate 3D map.}
	\label{fig:fusion}
\end{figure}

Les algorithmes de Slam Visuel monoculaire génèrent souvent des cartes de basse qualité à cause des difficultés de mise à l'échelle et de \textit{drift} angulaire. Afin de répondre à cette problématique la fusion permet de générer des résultats à partir des deux méthodes qui se complémentent l'une l'autre, comme c'est le cas dans l'exemple de la figure \ref{fig:fusion}. Tout comme on peut effectuer la fusion avec d'autres cartes du même milieu, dans Google maps\texttrademark on trouve souvent des plans exterieurs de batisses\cite{fusion}.
\subsection{Back End}
La partie \textit{back end} de SLAM rentre en jeu à partir du moment où on dispose de la pose relative, et qu'on a une base de donnée solide de scans issus d'un télémètre laser. Les scans, s'ils sont acceptés vont être incorporés das la structure de données du graphe de pose (voir section \ref{sec:graph-slam}), et la trajectoire consiste en une série de n\oe{}uds représentants les positions estimées du robots, tel que chaque pose correspond à un scan qui décrit l'environnement.\\
...
Il existe diveres alternatives à la SVD, mais c'est une mhode qui donne d'excellents résultats, qui est dtaillée dans l'annexe \ref{sec:annexe-b---la-decomposition-svd}.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=width=0.48]{icp7}
	\caption[bla]{blablabla}
	\label{fig:icp7}
\end{figure}
