...
\begin{itemize}
	...
	\item Sélection de points selon le poids de la normale (\texttt{MaxLeverageSampling}) ;
	...
	\item Sélection de points apparteants à un VoxelHull specifique, or des points appartnant à des parties communes à deux nuages de points, là où se produit un chevauchement (\texttt{InVoxelHull}) ;
...
\quad \textendash \quad L'angle entre les normales des points appareillés ne doit pas dépasser un certain seuil.

\quad \textendash \quad La distance point à point entre les points correspondants ne doit pas dépasser un certain seuil.

\quad \textendash \quad Une limite de distance pour les correspondances point à plan afin de filtrer les points aberrants (cette méthode est un estimateur médian (M-estimateur) robuste connu sous \textit{Generalised Median} (GM) pour une base de donnée supposée être conforme àun distribution gaussienne).

\quad \textendash \quad Maximum acceptable de la robustesse des plans extraits, et pour ce foire c'est la dérivée du plan qui est utilisée.
