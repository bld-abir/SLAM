...
% % % % % % % odomList = zeros(200,2);
% % % % % % % 
% % % % % % % for i = [1:200];
% % % % % % %     odomList(i,:)=[odomMsg{1, i}.Pose.Pose.Position.X odomMsg{1, i}.Pose.Pose.Position.Y]
% % % % % % %     i = i+1;
% % % % % % % end
% % % % % % % map = robotics.OccupancyGrid(odomList, 20);
% % % % % % % show(map)

maxLidarRange = 8;
mapResolution = 20;
slamAlg = robotics.LidarSLAM(mapResolution, maxLidarRange)
for i=1:length(laserMsg)
    currentScanMsg = laserMsg{i};
    currentOdomMsg = odomMsg{i};
    currentScan = lidarScan(currentScanMsg)
    [isScanAccepted, loopClosureInfo, optimizationInfo] = addScan(slamAlg, currentScan);
    if isScanAccepted
...
figure(1)
show(slamAlg);
title({'Map of the Environment','Pose Graph : Final Result'});    
...
%     % % %     odom = odomMsg(i);
%     % % %     odomList(i,:) = [odomMsg{1, i}.Pose.Pose.Position.X odomMsg{1, i}.Pose.Pose.Position.Y];
%     tab=[odomMsg{1, i}.Pose.Pose.Position.X odomMsg{1, i}.Pose.Pose.Position.Y]
% %     if i < 10
...
% currentScan = lidarScan(laserMsg{186});
% 
% %%
...
%     end
%       /home/blad/Bureau/MATLAB/R2018a/toolbox/robotics/robotexamples/robotalgs/ScanMatchingExample1.m
