% function [TR,TT,data] = icp2(model,data,TR, TT, maxIter,minIter,critFun,thres)
function [TR,TT,data, DT,res_vec] = icp2(model,data, maxIter,minIter,critFun,thres)
...
elseif nargin<8
    ...
    if nargin<7
        critFun=4;                  % critFun method, LS
        if nargin<6
            minIter=5;              % 5 min number of icp iterations
            if nargin<5
                maxIter=100;        % 100 max number of icp iterations
                if nargin<4
                   TR=eye(m);     % 
                   if nargin<3
                     TT=zeros(m,1);           % 
...
for iter=1:maxIter
    ...
    switch critFun
        ...
        case 0
            ...
            res=mean(resid.^2);  % la moyenne de la distance entre les points des scans data et leurs correspondants dans model            
            med=mean(data,2);
            if mTranspose
                mem=mean(model(vi,:),1);
